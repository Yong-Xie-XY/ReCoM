...
class RenderTool():
    ...
    def _render_sequences_helper(self, video_fname, cur_wav_file, v_list, stand, face, whole_body, transcript):
        ...
        if int(cv2.__version__[0]) < 3:
            ...
            print('cv2 >= 3')
            writer = cv2.VideoWriter(tmp_video_file.name, cv2.VideoWriter_fourcc(*'mp4v'), 30, (2400, 1440), True)      # adjust size

        center = np.mean(v_list[0][0], axis=0)

        r = pyrender.OffscreenRenderer(viewport_width=2400, viewport_height=1440)   # 优化模型精度
    ...
    def _render_sequences_helper2(self, video_fname, cur_wav_file, pred, frame):
        ...
        if int(cv2.__version__[0]) < 3:
            ...
            print('cv2 >= 3')
            writer = cv2.VideoWriter(tmp_video_file.name, cv2.VideoWriter_fourcc(*'mp4v'), 30, (2400, 1440), True)

        center = np.mean(pred[0], axis=0)

        r = pyrender.OffscreenRenderer(viewport_width=2400, viewport_height=1440)
