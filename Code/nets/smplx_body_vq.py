...
class TrainWrapper(TrainWrapperBaseClass):
    ...
    def __init__(self, args, config):
        ...
        if self.composition:
            ...
            self.g_hand = s2g_body(self.each_dim[2], embedding_dim=64, num_embeddings=config.Model.code_num, num_hiddens=1024,
                                   num_residual_layers=2, num_residual_hiddens=512).to(self.device)
            # self.g_face = s2g_body(self.each_dim[0]+self.each_dim[3], embedding_dim=64, num_embeddings=config.Model.code_num, num_hiddens=1024,
            #                        num_residual_layers=2, num_residual_hiddens=512).to(self.device)
        else:
    ...
    def init_optimizer(self):
        ...
        if self.composition:
            ...
            # self.g_face_optimizer = optim.Adam(
            #     self.g_face.parameters(),
            #     lr=self.config.Train.learning_rate.generator_learning_rate,
            #     betas=[0.9, 0.999]
            # )
        else:
    ...
    def state_dict(self):
        if self.composition:
            ...
            model_state = {
                ...
                # 'g_face': self.g_face.state_dict(),
                # 'g_face_optim': self.g_face_optimizer.state_dict(),
    ...
    def __call__(self, bat):
        ...
        # poses.shape:torch.Size([128, 165, 88])
        # expression = bat['expression'].to(self.device).to(torch.float32)
        # jaw_poses = poses[:,:3,:]    # 用来取下巴pose
        # expression.shape:torch.Size([128, 100, 88])
        ...
        # f_poses = torch.cat((jaw_poses,expression),dim = 1).permute(0, 2, 1)
        ...
        if self.composition:
            ...
            # loss_dict, loss = self.vq_train(f_poses[:, :], 'f', self.g_face, loss_dict, loss)
        else:
            loss = 0
    ...
    def vq_train(self, gt, name, model, dict, total_loss, pre=None):
        ...
        # elif name == 'f':
        #     optimizer_name = 'g_face_optimizer'
        elif name == 'g':
    ...
    def load_state_dict(self, state_dict):
        if self.composition:
            ...
            # self.g_face.load_state_dict(state_dict['g_face'])
        else:
